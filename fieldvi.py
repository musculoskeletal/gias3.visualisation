...
import scipy
from traits.api import HasTraits, Int, Range, Bool, Button, Color, Dict, Tuple, Str, Enum, List, Instance, on_trait_change
from traitsui.api import View, Item, HGroup, HSplit, VFlow, VGroup, Tabbed, CheckListEditor, RangeEditor, EnumEditor, ListEditor, InstanceEditor, TextEditor, Spring, spring
...
class Fieldvi(HasTraits):
                    ...
                    Tabbed(
	                    VGroup(
							...
							VGroup(	
								...
								HGroup(
									Item( 'GFScalarList0', show_label=True, label='GFScalars', editor = EnumEditor(name='GFScalarList') ),
							...
							VGroup(	
								...
								HGroup(
									Item( 'triScalarList0', show_label=True, label='triScalars', editor = EnumEditor(name='triScalarList') ),
	...
	def addTri( self, name, tri, renderArgs=None ):
		...
		if name not in self.triSurface.keys():
			self.triCounter += 1
	...
	def addGeometricField( self, name, G, evaluator, GD=None, renderArgs=None  ):
		...
		if name not in self.geometricFields.keys():
			self.GFCounter += 1
	...
	def _drawAxes(self):
		...
		self.sceneObjectData['origin'] = self.scene.mlab.points3d( o[:,0], o[:,1], o[:,2], mode='sphere', scale_factor=4.0)
	...
	def _refreshTriScalars_fired( self ):
		print 'refresh scalars!'
		self.triScalarList = self.triScalarData[self.triList0].keys()
	...
	def _renderTriSurfaces_fired( self ):
		...
		"""
		for i in self.sceneObjectTri.values():
			i.remove()
	...
	def _triCutPlane_fired( self ):
		...
		if self.sceneObjectTriCutPlanes.get(self.triList0):
			...
			self.sceneObjectTriCutPlanes[self.triList0] = mlab.pipeline.vector_cut_plane(self.sceneObjectTri[self.triList0],
																					   mask_points=1,
																					   mode='sphere',
																					   scale_factor=3)
	...
	def _GFCutPlane_fired( self ):
		...
		if self.sceneObjectGFCutPlanes.get(self.GFList0):
			...
			try:
				self.sceneObjectGFCutPlanes[self.GFList0].origin = [0,0,self.sceneObjectImages['0'].widgets[0].slice_index]
			except KeyError:
				pass
	...
	def updateTriSurface( self, name, coords=None ):
		...
		try:
			if scalar is None:
				...
				if renderArgs.has_key('color'):
					del renderArgs['color']
				
				self.sceneObjectTri[name].actor.mapper.scalar_visibility=True
				self.sceneObjectTri[name].mlab_source.set( x=V[:,0], y=V[:,1], z=V[:,2], scalars=self.triScalarData[name][self.triScalarList0], **renderArgs )
	...
	def _drawTriSurface( self, name ):
		...
		if S is None:
			...
			self.sceneObjectTri[name] = self.scene.mlab.triangular_mesh( P[0], P[1], P[2], T, scalars=S, name=name, **renderArgs )
	...
	def _getGFScalarData( self, scalarName, GFName ):
		...
		if scalarName in self.scalarData.keys():
	...
	def _drawGeometricField( self, name ):
		...
		renderArgs = self.GFRenderArgs[name]
		if 'color' not in renderArgs.keys():
			renderArgs['color'] = self.defaultColor
		...
		if g.ensemble_field_function.dimensions==2:
			...
			if (S is None) or (S=='none'):
				...
				# print S
				self.sceneObjectGF[name] = self.scene.mlab.triangular_mesh( P[0], P[1], P[2], T, scalars=S, name=name, **renderArgs )
	...
	def drawGeometricFieldElementNumbers( self, name, textScale=5.0, textColor=(0,0,0) ):
		...
		gElemLabels = [ self.scene.mlab.text3d( ex[i][0], ex[i][1], ex[i][2], str(ei[i]), color=textColor, scale=textScale ) for i in xrange(len(ei)) ]
		return gElemLabels
	...
	def _modeIndex_changed(self):
		...
		if q:
			...
			nodeCoord = self.pc.reconstruct( self.pc.getWeightsBySD( [self.modeIndex,], [self.mode1,] ), [self.modeIndex,] ).reshape((3,-1))
	...
	def _modeQuiver_fired( self ):
		...
		if q:
			...
			self.sceneObjectGF['popQuiver'] = mlab.quiver3d( nodeCoord[0], nodeCoord[1], nodeCoord[2], mNorm[0], mNorm[1], mNorm[2], scale_mode='none', scale_factor=5.0, mode='arrow', color=(0,0,0)  )
		
	def _SFModeWeight_changed(self):
		# reconstruct field parameters
		params = self.SFPC.reconstruct( self.SFPC.getWeightsBySD( [self.SFModeIndex,], [self.SFModeWeight,] ), [self.SFModeIndex,] )
		#~ V = self.GEval( params )
		#~ # update vertices
		#~ self.sceneObjectGF[self.pcGFName].mlab_source.set( x=V[0], y=V[1], z=V[2] )
	...
	def _GFSFModeWeight_changed(self):
		...
		weightsComb = self.GFSFPC.reconstruct( self.GFSFPC.getWeightsBySD( [self.GFSFModeIndex,], [self.GFSFModeWeight,] ), [self.GFSFModeIndex,] )
		#~ V = self.GEval( params )
		#~ # update vertices
		#~ self.sceneObjectGF[self.pcGFName].mlab_source.set( x=V[0], y=V[1], z=V[2] )
		...
		if self.GFSFPCComb:
			...
			GFParams = self.pc.reconstruct( GFPCWeights, range(len(self.GFSFGFRows)) )
		self.updateGeometricField( self.GFSFGFName, params=GFParams )
		
		if self.GFSFPCComb:
			SFParams = SFPCWeights
		else:
			SFParams = self.SFPC.reconstruct( SFPCWeights, range(len(self.GFSFSFRows)) )
	...
	def setLowerLimbAtlas(self, LL, gfEvalMaker):
		...
		for mn, m in self.LL.models.items():
